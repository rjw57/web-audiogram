...
import { fetchAndDecodeAudioData } from "./fetchWrappers";
...
export const renderAudiogram = async (options: RenderAudiogramOptions) => {
  ...
  const [decodedAudioBuffer, encodedMediaBuffer] = await Promise.all([
    fetchAndDecodeAudioData(audioSampleRate, audioUrl),
    (async () => {
      const response = await fetch(backgroundVideoUrl);
      if (!response.ok) {
        throw new Error("Error fetching background video.");
      }
